...
int my_main_encrypt(int mode, char *keyword, char *infile) {
  int r;
  FILE *f;
...
//  cmd = read_commandline(ac, av);
  ...
  /* if --keyfile requested, read one (normal mode) or two (change key
	...
	//   if (cmd.keyfile) {
	// 
	//   printf("let's read the keyfile %s \n", cmd.keyfile);
	//   
	...
	// 
	// 
	//     printf("let's read the keyword %s \n", cmd.keyword);
	// 
  ...
  //  printf("in encrypt\n");
    ...
    free(cmd.keyword);
    cmd.keyword = keyword;
//            readkey(cmd.prompt, "", cmd.name, 0);
	
    //printf("in getting encrypt %s\n", cmd.keyword);
...
//    }
  ...
  /* read keyword2 from terminal if necessary....not being used */
	//   if (cmd.mode==KEYCHANGE && cmd.keyword2==NULL) {
	// 
	//   printf("let's read the keyword222 ");
	//     if (cmd.prompt2 == NULL) {
...
//  setmode(1,O_BINARY);
  ...
  /* if --keyref given, check encryption keys against named file  NOT BEING USED*/
	//   if (cmd.keyref && (cmd.mode == ENCRYPT || cmd.mode == KEYCHANGE)) {
	//   
	//   printf("let's read the keyref %s \n", cmd.keyref);
	// 
  ...
  /* filter mode NOT BEING USED */

  // if (cmd.filter) {   
//   	printf("in filter shit \n");
// 	  
//     switch (cmd.mode) {  
// 
//     case ENCRYPT: default:
...
//   }
 ...
 // printf("in the end of the file 2\n");
