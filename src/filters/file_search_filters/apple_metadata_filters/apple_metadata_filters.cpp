...
apple_metadata_filters::apple_metadata_filters(QWidget *parent) : QWidget(parent)
{
...
//                      << MACRO_Generic_Filter_Condition_Last_Modified_Date << MACRO_Generic_Filter_Condition_Others;
    ...
    list_search_type_kind_search_items << MACRO_Generic_Filter_Condition_Matches << MACRO_Generic_Filter_Condition_Contains
                                       << MACRO_Generic_Filter_Condition_Begins_With << MACRO_Generic_Filter_Condition_Ends_With
                                       << MACRO_Generic_Filter_Condition_Is_Not;
    ...
    list_search_type_date_search_items  << MACRO_Generic_Filter_Condition_Exactly
                                        << MACRO_Generic_Filter_Condition_Before << MACRO_Generic_Filter_Condition_After;
...
void apple_metadata_filters::pub_set_essentials()
{
    db_path_apple_attributes = global_narad_muni_class_obj->get_field(MACRO_NARAD_Conf_Path_Location_Case_Configuration_In_Result_QString).toString() + "case_configuration.sqlite";

    QString attributes_command_str = QString("Select attribute_name From tbl_apple_attributes Where bookmark_filter = '1' or bookmark_report = '1'");
    list_search_types = recon_helper_standard_obj->get_stringlist_from_db_by_dbpath(attributes_command_str, 0 ,db_path_apple_attributes , Q_FUNC_INFO);
    ...
    for(int i = 0 ; i < vlayout_filters->count(); i++)
    {
        ...
        for(int j = 0; j < item->layout()->count() ; j++)
        {
            ...
            if(item2->widget() == combobox_search_type_filter)
            {
                indexoditem = i;
                break;
            }
    ...
    set_range_interface(indexoditem,combobox_search_type_filter->currentText());
}

void apple_metadata_filters::pub_create_ui()
{
    ...
    combobox_search_type_filter = new QComboBox(this);
...
void apple_metadata_filters::on_apl_attribute_ok_button_click()
{

    list_search_types =  get_main_list_from_apl_attribute_database();
...
QStringList apple_metadata_filters::get_main_list_from_apl_attribute_database()
{
...
void apple_metadata_filters::on_apl_attribute_cancel_button_click()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
    ...
    if(!destination_db.open())
    {
        recon_static_functions::app_debug(" Destination Db opening --------FAILED------ " + db_path_apple_attributes,Q_FUNC_INFO);
...
int apple_metadata_filters::sub_set_all_index()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
int apple_metadata_filters::get_main_combobox_index(QLayoutItem *item_combobox)
{
    int index_main = 0;
...
QString apple_metadata_filters::pub_collect_all_spotlight_search_filters_information()
{
    recon_static_functions::app_debug(" Starts" , Q_FUNC_INFO);
    ...
    recon_static_functions::app_debug(" End -- " + mdfind_condition , Q_FUNC_INFO);

    return mdfind_condition;
}

void apple_metadata_filters::pub_set_all_filter_status(bool staus)
{
    bool_all_filter_status = staus;
}

QString apple_metadata_filters::make_strings_for_mdfind_search_condition(QStringList list)
{
    recon_static_functions::app_debug(" Starts" , Q_FUNC_INFO);

    QString mdfind_conditions;
    QStringList apple_metadata_query_filters;
    ...
    if(!destination_db.open())
    {
        recon_static_functions::app_debug("destination_db Open -------FAILED------ " + db_path_apple_attributes , Q_FUNC_INFO);
        recon_static_functions::app_debug(destination_db.lastError().text() , Q_FUNC_INFO);
    ...
    for(int i = 0 ; i < list.size() ; i++)
    {
        ...
        query.prepare("select widget_type, custom_column_name from tbl_apple_attributes where attribute_name = ?");
        query.addBindValue(name);
        if(!query.exec())
        {
        ...
        {
            apple_metadata_query_filters <<  get_mdkind_condition_str_for_widget_type_name(sub_list,command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_OPTIONS)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_options(sub_list,command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_RANGE)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_range(sub_list,command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_REAL_VALUE)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_real_value(sub_list,command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_CONTENT)
        {
            QStringList sub_list = name_str.split(MACRO_RECON_Splitter_3_dollar_in_brace, Qt::SkipEmptyParts);
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_content(sub_list,command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_KIND)
        {
            QStringList sub_list = name_str.split(MACRO_RECON_Splitter_3_dollar_in_brace, Qt::SkipEmptyParts);

            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_kind(sub_list);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_DATE)
        {
            QStringList sub_list = name_str.split(MACRO_RECON_Splitter_3_dollar_in_brace, Qt::SkipEmptyParts);

            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_date(sub_list, command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_Duration)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_duration(sub_list, command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_SIZE)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_size(sub_list, command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_SIMPLE_LINEEDIT)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_simple_lineedit(sub_list, command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_SIMPLE_LABEL_N_LINEEDIT)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_simple_label_and_lineedit(sub_list, command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_SINGLE_SYSTEM_COMBOBX)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_single_system_combobox(sub_list, command);
        }
        else if(type == enum_File_System_Widget_Type_FILESYSTEM_SINGLE_VISIBLE_COMBOBX)
        {
            apple_metadata_query_filters << get_mdkind_condition_str_for_widget_type_single_visible_combobox(sub_list, command);
        }
    }

    QString string_all_filter_status;
    if(bool_all_filter_status)
        string_all_filter_status = " AND ";
    else
        string_all_filter_status = " OR ";

    mdfind_conditions = " " + apple_metadata_query_filters.join(string_all_filter_status) + " ";
    if(mdfind_conditions.endsWith(string_all_filter_status))
        mdfind_conditions.chop(string_all_filter_status.size());

    destination_db.close();
    return mdfind_conditions;
}

QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_name(QStringList sub_name_list, QString command)
{
    ...
    else if(match1 == MACRO_Generic_Filter_Condition_Matches)
    {
        mdkind_conditions.append(QString( command + " = '" + match_str ) + "' ");
    }
    else if(match1 == MACRO_Generic_Filter_Condition_Contains)
    {
        mdkind_conditions.append(QString( command + " LIKE '%" + match_str )+ "%' ");
    }
    else if(match1 == MACRO_Generic_Filter_Condition_Begins_With)
    {
        mdkind_conditions.append(QString( command + " LIKE '" + match_str )+ "%' ");
    }
    else if(match1 == MACRO_Generic_Filter_Condition_Ends_With)
    {
        mdkind_conditions.append(QString( command + " LIKE '%" + match_str )+ "' ");
    }
    else if(match1 == MACRO_Generic_Filter_Condition_Is_Not)
    {
        mdkind_conditions.append(QString( command + " is not '" + match_str) + "' ");
    }

    return mdkind_conditions;
}

QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_options(QStringList sub_name_list, QString command)
{
    QString mdkind_conditions;
    ...
    if(mdkind_conditions.endsWith(" && "))
        mdkind_conditions.chop(QString(" && ").size());


    return mdkind_conditions;
}

QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_real_value(QStringList sub_name_list, QString colmn_name)
{
    QString mdkind_conditions;
    ...
    if(match_str.trimmed() == QString(""))
        return mdkind_conditions;


    QString cast_string_value_to_integer_str = " CAST(" + colmn_name + " as INTEGER)";

    if(match1 == MACRO_Generic_Filter_Condition_Equal && match_str != "")
    {
        mdkind_conditions.append(QString(cast_string_value_to_integer_str + " = "  + match_str ) + " ");
    }
    ...
    QString cmd = " AND "+ colmn_name + " != '" + "'" ;

    mdkind_conditions += cmd;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_range(QStringList sub_name_list, QString command)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_content(QStringList sub_name_list, QString command)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_kind(QStringList sub_name_list)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_date(QStringList sub_name_list, QString command)
{
    QString mdkind_conditions;
    ...
    QDate mdate  = QDate::fromString(match_third, "yyyy/MM/dd");
    QDateTime mdt;
    mdt.setTimeSpec(Qt::UTC);
    mdt.setDate(mdate);
    mdt.setTime(QTime(0,0,0));
    mdt.setTimeSpec(Qt::UTC);

    uint m_epch = mdt.toSecsSinceEpoch();

    match_third = QString::number(m_epch);

    if(match_str == MACRO_Generic_Filter_Condition_Exactly)
    {
        mdt.setTime(QTime(23,59,59));
        mdt.setTimeSpec(Qt::UTC);
        uint epch_1 = mdt.toSecsSinceEpoch();
        mdkind_conditions.append(command + " between " + match_third + " AND " +  QString::number(epch_1));
    }
    else if(match_str == MACRO_Generic_Filter_Condition_Before)
    {
        mdkind_conditions.append(command + " < " + match_third + " ");
    }

    else if(match_str == MACRO_Generic_Filter_Condition_After)
    {
        mdkind_conditions.append(command + " > " + match_third + " ");
    }

    mdkind_conditions += " AND " + command + "!='" + "'";


    return mdkind_conditions;
}

QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_duration(QStringList sub_list, QString command)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_size(QStringList sub_list, QString command)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_simple_lineedit(QStringList sub_list, QString command)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_simple_label_and_lineedit(QStringList sub_list, QString command)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_single_system_combobox(QStringList sub_list, QString command)
{
    QString mdkind_conditions;
...
QString apple_metadata_filters::get_mdkind_condition_str_for_widget_type_single_visible_combobox(QStringList sub_list, QString command)
{
    QString mdkind_conditions;
