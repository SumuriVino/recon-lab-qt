...
bookmarks_search_loader::bookmarks_search_loader(QWidget *parent)
{
...
bookmarks_search_loader::~bookmarks_search_loader()
{

}

void bookmarks_search_loader::pub_bookmarks_search_set_display_db_path(QString db_path)
{
    destination_db_path_for_tag_search = db_path;
}

void bookmarks_search_loader::pub_set_global_connection_manager_obj(global_connection_manager *obj)
{
    global_connection_manager_obj = obj;
}

void bookmarks_search_loader::bookmarks_search_populate_data_in_table()
{
    ...
    record_no_column_name = db_columns_list.at(record_no_index);
    plugin_column_name = db_columns_list.at(plugin_name_index);
    tab_column_name = db_columns_list.at(tab_name_index);
    os_scheme_column_name = db_columns_list.at(os_name_index);

    QStringList menuList;
    menuList << QString(MACRO_Generic_Right_Click_Remove_Bookmarks);
...
void bookmarks_search_loader::contextMenuEvent(QContextMenuEvent *evt)
{
    if(evt == NULL)
        return;
    ...
    else
    {
        ...
        else if(plugin_name_str == QString(MACRO_Plugin_Name_Saved_Graphs_Browser_History) || plugin_name_str == QString(MACRO_Plugin_Name_Saved_Graphs_Messenger))
        {
            submenu_send_to_bucket->setEnabled(false);
            submenu_open_with->setEnabled(false);
            submenu_add_files_to_hashset_db->setEnabled(false);
    ...
    for(int count = 0; count < menu_list.size(); count++)
    {
        ...
        {
            if(plugin_name_str == QString(MACRO_Plugin_Name_Carved_Files) || plugin_name_str == QString(MACRO_Plugin_Name_Registry_Viewer)
                    || plugin_name_str ==QString(MACRO_Plugin_Name_Email_Parser)
                    || plugin_name_str == QString(MACRO_Plugin_Name_Carved_Password) ||plugin_name_str ==QString(MACRO_Plugin_Name_Mobile_Backup)
...
void bookmarks_search_loader::tablewidget_general_clicked(int row,int column)
{
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Ram_Analysis_Saved))
    {
        ...
        {
            recon_static_functions::app_debug(" ---FAILED--- to open DB" + bookmark_db_path,Q_FUNC_INFO);
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Sqlite_Viewer))
    {
        ...
        struct_global_fetch_metadata struct_metadata_values_obj;
        struct_metadata_values_obj = recon_helper_standard_obj->fetch_metadata_for_sqlite_viewer_tag_entry(filepath,QString(MACRO_Tag_Name_Bookmarks),record_no,db_table_name, source_count_name,Q_FUNC_INFO);
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Email_Parser))
    {
        ...
        QString command = QString("select INT,plugin_name,item0,item1,item2,item3,item4,item5,recon_tag_value,notes,tab_name,item9 from tags where record_no = ? AND tab_name = ? AND plugin_name = ?");
        query_select.prepare(command);
        query_select.addBindValue(record_no);
        query_select.addBindValue(tab_name_str);
        query_select.addBindValue(plugin_name_str);
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Plist_Viewer))
    {
        ...
        struct_global_fetch_metadata struct_metadata_values_obj = recon_helper_standard_obj->fetch_metadata_for_plist_viewer_tag_entry(filepath,"",viewer_display_name, record_no,source_count_name, Q_FUNC_INFO);
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Carved_Files))
    {
        ...
        struct_global_fetch_metadata struct_metadata_values_obj;
        struct_metadata_values_obj = recon_helper_standard_obj->fetch_metadata_for_carved_files(carved_files_db_path, record_no,Q_FUNC_INFO);
        metadata_value_str = struct_metadata_values_obj.metadata;
    ...
    {
        struct_global_fetch_metadata struct_metadata_values_obj = recon_helper_standard_obj->fetch_metadata_of_plugin(plugin_name_str,tab_name_str,record_no,global_narad_muni_class_obj->get_field(MACRO_NARAD_RECON_Result_Dir_QString).toString(), Q_FUNC_INFO);
        metadata_value_str = struct_metadata_values_obj.metadata;
        final_plugin_name = plugin_name_str;
        finaltab_name = tab_name_str;
...
void bookmarks_search_loader::action_open_full_detailed_info_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_open_detach_detailed_info_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::update_notes_value(QString text, int current_row)
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
    ...
    if(table_name_list.size() > 0)
    {
        ...
        else if(plugin_name_str == QString(MACRO_Plugin_Name_Plist_Viewer))
        {
            ...
            global_connection_manager_obj->pub_set_notes_in_tags_search_for_plist_viewer(INT_list,display_tab_name ,text);
        }
        ...
        else if(plugin_name_str == QString(MACRO_Plugin_Name_Sqlite_Viewer))
        {
            ...
            global_connection_manager_obj->pub_set_notes_in_tags_search_for_sqlite_viewer(INT_list,tab_name_str,db_table_name,result_tab_name,text);
        }
        else
        {
...
void bookmarks_search_loader::action_sendToBucket_hex_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_sendToBucket_sqlite_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_sendToBucket_plist_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_sendToBucket_log_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_open_with_log_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
    ...
    else
    {
        ...
        if(list_log_display_name.contains(log_display_nm))
        {
            emit signal_show_file_in_viewer_table_result(log_display_nm,MACRO_CASE_TREE_Log_Viewer, source_count_name);
        }
        else
        {
            log_viewer *log_viewer_obj;
...
void bookmarks_search_loader::action_open_with_plist_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_open_with_hex_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_open_with_sqlite_viewer_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::action_export_triggered()
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
    ...
    for(int i = 0; i < selection_model_list.size(); i++)
    {
        ...
        else if(plugin_name_str == QString(MACRO_Plugin_Name_Carved_Files))
        {
            ...
            QString complete_path = global_recon_helper_singular_class_obj->get_complete_file_path(result_dir_path,filepath,Q_FUNC_INFO);
...
QStringList bookmarks_search_loader::export_file_system_record(QString record_no,int current_row)
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::create_action_for_goto_artifact_source_submenu()
{
    ...
    if(plugin_name_str == QString(MACRO_Plugin_Name_File_System) || plugin_name_str == QString(MACRO_Plugin_Name_Hex_Viewer)
            ...
            || plugin_name_str == QString(MACRO_Plugin_Name_Browser_History_Full_RR) ||plugin_name_str == QString(MACRO_Plugin_Name_Messenger_Full_RR)
            || plugin_name_str == QString(MACRO_Plugin_Name_Email_Parser)
            ...
            || plugin_name_str == QString(MACRO_Plugin_Name_Carved_Data) || plugin_name_str ==QString(MACRO_Plugin_Name_Snapshots))
        return;
    else
    {
...
void bookmarks_search_loader::action_go_to_record_triggered()
{
    recon_static_functions::app_debug(" -Starts " , Q_FUNC_INFO);
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Sqlite_Viewer) || plugin_name_str == QString(MACRO_Plugin_Name_Plist_Viewer))
    {
        QString source_count_name = m_tablewidget_obj->item(current_row, source_count_name_index)->text();
        ...
        {
            file_path = recon_helper_standard_obj->get_string_from_db_with_addbindvalues_by_dbpath("select filepath from files where id_path = ?" , QStringList(file_path), 0, fs_db_path, Q_FUNC_INFO);
        }
...
void bookmarks_search_loader::action_remove_note_triggered()
{
    QModelIndexList selection_model_list = m_tablewidget_obj->selectionModel()->selectedRows();
...
void bookmarks_search_loader::slot_action_export_all_file_system_records_triggered()
{
...
void bookmarks_search_loader::slot_action_export_selected_file_system_records_triggered()
{
...
void bookmarks_search_loader::action_remove_bookmark_triggered()
{
    recon_static_functions::app_debug("-Starts " , Q_FUNC_INFO);
...
void bookmarks_search_loader::update_bookmark_value(QString status,int row, int column)
{
    recon_static_functions::app_debug(" -Starts " , Q_FUNC_INFO);
    ...
    if(plugin_name_str == QString(MACRO_Plugin_Name_File_System))
    {
        ...
        global_connection_manager_obj->pub_remove_bookmark_in_fs_source_db(m_list, source_count_name);
    ...
    {
        global_connection_manager_obj->pub_remove_bookmark_in_redefined_db(QStringList(record_no) , plugin_name_str);
        global_connection_manager_obj->pub_delete_bookmark_for_redefined_db(QStringList(record_no) , plugin_name_str);
        return;
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Email_Parser))
    {
        ...
        arg_list.clear();
        command = "UPDATE emlx_data SET bookmark=?  WHERE INT = ?";
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Artifacts_Timeline))
    {
        ...
        global_connection_manager_obj->pub_remove_bookmark_in_artifact_timeline_db_from_outside(artifact_timeline_record_list);
        global_connection_manager_obj->pub_delete_bookmark_for_artifact_timeline(artifact_timeline_record_list);
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Plist_Viewer))
    {
        ...
        QString plist_db_table_name = m_tablewidget_obj->item(row, 12)->text();
        QString source_count_name = m_tablewidget_obj->item(row, source_count_name_index)->text();
        ...
        QString cmd = "select recon_tag_value , notes from plist_content_info where viewer_display_name = ? AND file_path = ? AND source_count_name = ? AND record_no = ?";
        arg_list << viewer_display_name << file_path << source_count_name << record_no;
        QString tag_name =  recon_helper_standard_obj->get_string_from_db_with_addbindvalues_by_dbpath(cmd,arg_list,0,plist_content_db,Q_FUNC_INFO);
        ...
        if(tag_name.trimmed().isEmpty() && notes_text.trimmed().isEmpty())
            ...
            global_connection_manager_obj->pub_update_bookmark_export_content_plist_viewer_db(QStringList(record_no) ,viewer_display_name);

        global_connection_manager_obj->pub_remove_bookmark_in_plist_viewer_db(QStringList(record_no) ,plist_db_table_name);
        global_connection_manager_obj->pub_delete_bookmark_for_plist_viewer(QStringList(record_no) ,viewer_display_name);
        return ;
    }
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Sqlite_Viewer))
    {
        ...
        QString db_table_name = m_tablewidget_obj->item(row, sqlite_viewer_db_table_name_index)->text();
        QString filepath = m_tablewidget_obj->item(row, file_path_index)->text();
        QString display_tab_name = m_tablewidget_obj->item(row, sqlite_viewer_diplay_name_index)->text();


        ///------------update export content sqlite db
        cmd = "select recon_tag_value , notes from sv_content_info where file_path = ? AND record_no_in_sqliteviewer = ? AND db_table_name = ? AND sql_tab_name = ?";
        arg_list << filepath << record_no << db_table_name << tab_name_str ;
        QString tag_value =  recon_helper_standard_obj->get_string_from_db_with_addbindvalues_by_dbpath(cmd,arg_list,0,sqlite_content_db,Q_FUNC_INFO);

        QString notes_value =  recon_helper_standard_obj->get_string_from_db_with_addbindvalues_by_dbpath(cmd,arg_list,1,sqlite_content_db,Q_FUNC_INFO);


        arg_list.clear();
        if(tag_value.trimmed().isEmpty() && notes_value.trimmed().isEmpty())
        {
            cmd = "delete from sv_content_info where file_path = ? AND record_no_in_sqliteviewer = ? AND db_table_name = ? AND sql_tab_name = ?";
            arg_list << filepath << record_no << db_table_name << tab_name_str;

        }
        else
        {
            cmd = "update sv_content_info set bookmark = ? where file_path = ? AND record_no_in_sqliteviewer = ? AND db_table_name = ? AND sql_tab_name = ?";
            arg_list  << status << filepath << record_no << db_table_name << tab_name_str;
        }
        recon_helper_standard_obj->execute_db_command_with_addbindvalues_by_dbpath(cmd,arg_list,sqlite_content_db,Q_FUNC_INFO);
        ...
        cmd = QString("select savedstate_dest_dir_path from sv_index where viewer_display_name = ?");
        arg_list.clear();
        ...
        cmd = "update '" + db_table_name + "' set RECON_bookmark = ? where RECON_INT=?";
        arg_list.clear();
        arg_list << status << record_no;
    ...
    {
        QString result_tab_name = m_tablewidget_obj->item(row,9)->text();
        global_connection_manager_obj->pub_remove_bookmark_in_carved_files_db(QStringList(record_no) , result_tab_name);
        global_connection_manager_obj->pub_delete_bookmark_for_carved_files_db(QStringList(record_no) , result_tab_name);
        return;
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Carved_Password))
    {
        ...
        QString result_tab_name = m_tablewidget_obj->item(row,10)->text();
        global_connection_manager_obj->pub_remove_bookmark_in_carved_password_db(QStringList(record_no) , result_tab_name);
        global_connection_manager_obj->pub_delete_bookmark_for_carved_password_db(QStringList(record_no) , result_tab_name);
        return;
    ...
    {
        QString result_tab_name = m_tablewidget_obj->item(row,17)->text();
        global_connection_manager_obj->pub_remove_bookmark_in_registry_viewer_db(QStringList(record_no) , result_tab_name);
        global_connection_manager_obj->pub_delete_bookmark_for_registry_viewer_db(QStringList(record_no) , result_tab_name);
        return;
    }
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Mobile_Backup))
    {
        global_connection_manager_obj->pub_remove_bookmark_in_Mobile_Backup_db(QStringList(record_no));
        global_connection_manager_obj->pub_delete_bookmark_for_Mobile_Backup_db(QStringList(record_no));
        return;
    }
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Carved_Data))
    {
        QString source_count_name = m_tablewidget_obj->item(row, source_count_name_index)->text();
        QString category_name = m_tablewidget_obj->item(row, item_11_index)->text();
        QString db_table_name = m_tablewidget_obj->item(row, item_12_index)->text();
        ...
        QString cmd = "select notes , recon_tag_value from tbl_carved_data where record_no = ? AND category_name = ? AND source_count_name = ?";
        QStringList arg_list;
        ...
        if(notes_text.trimmed().isEmpty() && tag_name.trimmed().isEmpty())
            tagging_on_record = MACRO_FALSE_VALUE_NUMERIC_QString;
        else
            tagging_on_record = MACRO_TRUE_VALUE_NUMERIC_QString;


        global_connection_manager_obj->pub_update_bookmark_export_content_carved_data_db(QStringList(record_no),category_name,QStringList(source_count_name),MACRO_FALSE_VALUE_NUMERIC_QString);
        ...
        global_connection_manager_obj->pub_remove_bookmark_in_carved_data_db(QStringList(record_no),db_table_name);
    ...
    else if(plugin_name_str == QString(MACRO_Plugin_Name_Snapshots))
    {
        ...
        global_connection_manager_obj->pub_remove_bookmark_in_snapshots_difference_db(QStringList(record_no) ,snapshot_label_name,global_csv_reader_class_selected_obj->get_table_name_according_tab(plugin_name_str,tab_name_str));
