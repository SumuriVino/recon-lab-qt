...
void notes_search_loader::slot_hashset_storage_workdone_clicked(QStringList list_dbs_addmenu, QString callfrom_str)
{
...
void notes_search_loader::slot_add_files_to_hashset_submenu_clicked(QAction* current_clicked_action)
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void notes_search_loader::slot_goto_artifact_source_submenu_clicked(QAction *clicked_action)
{
...
void notes_search_loader::slot_sort_tablewidget_display(int column_index)
{
    recon_static_functions::app_debug("Starts " , Q_FUNC_INFO);
    ...
    //    if(column_index == 0)
    //    {
    //        m_tablewidget_obj->sortByColumn(column_index);
    //        m_tablewidget_obj->setSortingEnabled(false);
...
void notes_search_loader::slot_action_export_all_file_system_records_triggered()
{
...
void notes_search_loader::slot_action_export_selected_file_system_records_triggered()
{
    bool_export_all_file_system_records = false;
    file_export_dialog_obj->show();
}

void notes_search_loader::slot_export_file_dialog_work_done(bool bool_keep_directory_structure, QString export_dir_path)
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
...
void notes_search_loader::slot_thread_zip_file_export_finished()
{
    display_loading_progress_bar_obj->hide();
...
void notes_search_loader::slot_export_file_dialog_work_done_for_zip(QString export_zip_file_path,bool bool_is_enc_zip,QString password_enc_zip)
{
    recon_static_functions::app_debug(" -Ends " , Q_FUNC_INFO);
...
void notes_search_loader::slot_export_file_dialog_work_done_for_dmg(bool bool_keep_directory_structure, QString export_dmg_file_path)
{
    recon_static_functions::app_debug(" Starts " , Q_FUNC_INFO);
    ...
    for(int i = 0; i < rows_list_to_export.size(); i++)
    {
        ...
        if(bool_keep_directory_structure)
        {
            dest_dir_path = export_dmg_file_path  + source_count_name + "_" + current_timestamp + "/" + parent_dir + "/";
            QDir mdir;
            mdir.mkpath(dest_dir_path);
        }
        else
        {
            dest_dir_path = export_dmg_file_path  + source_count_name + "_" + current_timestamp + "/";
            QDir mdir;
            mdir.mkpath(dest_dir_path);
        }
...
void notes_search_loader::slot_message_box_yes_no_clicked(bool yes_no_clicked)
{
...
void notes_search_loader::point_to_export_dir(QString path)
{
    QStringList arg;
    arg << "-R" << path;
...
void notes_search_loader::action_goto_source_triggered()
{
    QString plugin_name_str = m_tablewidget_obj->item(m_tablewidget_obj->currentRow(), plugin_name_index)->text();
    QString file_path ;
