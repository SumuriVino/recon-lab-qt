...
class tag_search_loader : public table_result_loader
{
public:
    tag_search_loader(QWidget *parent = 0);
    ~tag_search_loader();

    void pub_tag_search_set_display_db_path(QString db_path);

    void tag_search_populate_data_in_table();


    void pub_tag_search_set_current_tag(QString current_tag);
private slots:
...
private:
    ...
    QString current_tag_hex_value;

    QString current_tag_clicked;
   // int populate_data_in_tablewidget(QString database_path, QString db_cmd, QStringList tab_list_from_csv);
    ...
    int temp_file_path_text_viewer_index;
    int item_0_index;
    int item_1_index;
    int item_2_index;
    int item_3_index;
