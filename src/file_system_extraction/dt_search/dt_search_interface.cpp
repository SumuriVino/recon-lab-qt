...
qint64 global_dt_indexing_processed_files_count = 0;
bool bool_global_dt_cancel_extraction = false;
...
void dt_search_interface::init()
{
    ...
    m_options.binaryFiles = dtsoFilterBinaryUnicode;
    m_options.textFlags   = dtsoTfDatabasesAsText | dtsoTfSkipNumericValues | dtsoTfXmlAsText;
    m_options.hyphens     = dtsoHyphenAsSpace;
...
void dt_search_interface::allocResults()
{
    ...
    //m_searchJob->maxFilesToRetrieve = 10000000000;
}
...
qint64 dt_search_interface::pub_get_processed_files_count()
{
    return global_dt_indexing_processed_files_count;
}

void dt_search_interface::pub_set_bool_cancel_extraction(bool status)
{
    bool_global_dt_cancel_extraction = status;
}

int dt_search_interface::indexingCB(void * pData, struct dtsMessage& msg)
{
    if(bool_global_dt_cancel_extraction)
    {
        return dtsAbort;
    }
    ...
    case dtsnIndexStartingFile:
    {
        ...
        if(!QString(prog->file.name).trimmed().isEmpty() && !QString(prog->file.name).trimmed().endsWith(".extext"))
        {
            global_dt_indexing_processed_files_count++;
        }
...
{
    m_indexJob->clear();
    QString index_name = "Index_1_" + QString::number(rand()) + QString::number(QDateTime::currentMSecsSinceEpoch());
...
bool dt_search_interface::updateIndex(int fClearFirst, QString dir_to_index, QString txt_file_with_paths)
{
    ...
    if (dir[0] == '@')  // list of filenames
        strCopy(m_indexJob->toAdd.fileListName, txt_file_with_paths.toLocal8Bit().data(), sizeof m_indexJob->toAdd.fileListName);
    else
    {
...
void dt_search_interface::showItem(dtsSearchResultsItem& item)
{
    ...
    QString indx_path = m_searchJob->indexesToSearch;
    QString source_count_name;
    QStringList tmp_list = indx_path.split("/", Qt::SkipEmptyParts);
    ...
    {
        QString item_location = item.location;
        QString id_path;
        QString file_pathh;
        if(item_location.contains(" --> "))
        {
            QStringList tmp_item_location = item_location.split(" --> ");
            if(tmp_item_location.size() > 0)
            {
                id_path = tmp_item_location.at(0);
            }
        }
        else
        {
            id_path = item.location;
        }
        ...
        if(id_path.startsWith(struct_target_info.virtual_source_path))
            id_path.remove(0, struct_target_info.virtual_source_path.size());

        if(!id_path.isEmpty() && !id_path.startsWith("/"))
            id_path.prepend("/");

        file_pathh = recon_helper_standard_obj->get_string_from_db_with_addbindvalues_by_dbpath(QString("Select filepath from files where id_path = ?"), QStringList(id_path), 0, fs_db_path, Q_FUNC_INFO);

        if(file_pathh.endsWith(".zip",Qt::CaseSensitive) || file_pathh.endsWith(".rar",Qt::CaseSensitive)
                || file_pathh.endsWith(".tar",Qt::CaseSensitive)  || file_pathh.endsWith(".gz",Qt::CaseSensitive))
        {
            struct_info.file_name = QFileInfo(QString(item.location)).fileName();
            struct_info.file_path = id_path;
        }
        else
        {
            struct_info.file_name = QFileInfo(QString(item.filename)).fileName();
            struct_info.file_path = QString(item.filename);
        }
    }
    else
    {
        if(QString(item.filename).endsWith(".extext"))
        {
            struct_info.file_name = QFileInfo(QString(item.location)).fileName();
            struct_info.file_path = item.location;
        }
        else if(QString(item.filename).contains(".zip",Qt::CaseSensitive) || QString(item.filename).contains(".rar",Qt::CaseSensitive)
                || QString(item.filename).contains(".tar",Qt::CaseSensitive) || QString(item.filename).contains(".gz",Qt::CaseSensitive))
        {
            //item.filename = /Volumes/Out put/new_source/RECON_LAB.zip>20e4d5,f5|RECON_LAB/z_mainwindow/load_case/mainwindow_load_case.cpp
            struct_info.file_name = item.shortName;
            QString item_location = item.location;
            if(item_location.contains(" --> "))
            {
                item_location = item_location.replace(" --> ","/");
                struct_info.file_path = item_location;
            }
            else
            {
                struct_info.file_path = item_location;
        ...
        {
            //Here we are using 'filename' instead of 'displayname' becuase sometime 'displyname' gives the name which is not the current name of file
            //filename =  /Users/karan/Desktop/Demo/test/NNEDV_SpyWareAndSafety_English.pdf
            //displayname = Microsoft Word - NNEDV_SpyWareAndSafety_English.doc

            //        struct_info.file_name = item.shortName;
            //        struct_info.file_path = QString(item.location) + QString("/") + QString(item.displayName);
