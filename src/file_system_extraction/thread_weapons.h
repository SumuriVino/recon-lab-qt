#ifndef THREAD_WEAPONS_H
#define THREAD_WEAPONS_H
...
class thread_weapons : public QObject
{
    Q_OBJECT
public:

    explicit thread_weapons(QObject *parent = nullptr);
    ~thread_weapons();
...
signals:
    ...
    void finished();
    void signal_add_weapons_to_case_tree(QStringList weapons_db_path);
...
public slots:
    void slot_weapons_extraction();

private slots:
    void slot_process_weapons_finished(int status);
    void slot_process_weapons_readyread();
...
private:
    ...
    bool bool_weapons_extraction_process_finished = false;
    bool bool_cancel_extraction_weapons = false;
