#include "text_viewer_aascii_unicode.h"
#include "ui_text_viewer_aascii_unicode.h"

text_viewer_aascii_unicode::text_viewer_aascii_unicode(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::text_viewer_aascii_unicode)
{
    ui->setupUi(this);

    recon_helper_standard_obj = new recon_helper_standard(this);
    ui->textBrowser_text->setUndoRedoEnabled(true);

    ui->lineEdit_search->setAttribute(Qt::WA_MacShowFocusRect, false);
    ui->lineEdit_search->setClearButtonEnabled(true);

    ui->textBrowser_text->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(ui->textBrowser_text, SIGNAL(customContextMenuRequested(const QPoint&)),this, SLOT(ShowContextMenu(const QPoint&)));
...
text_viewer_aascii_unicode::~text_viewer_aascii_unicode()
{
    delete ui;
}


void text_viewer_aascii_unicode::ShowContextMenu(const QPoint& pos)
{
    QPoint globalPos = ui->textBrowser_text->mapToGlobal(pos);
    mainMenu->exec(globalPos);
}


void text_viewer_aascii_unicode::pub_clear_textview()
{
    //ui->textBrowser_text->setForeground(QColor(Qt::black));

    ui->textBrowser_text->clear();
    file_data_bytearray.clear();
    file_data_ascii.clear();
    file_data_unicode.clear();


}

void text_viewer_aascii_unicode::pub_set_essentials()
{
    create_tags_submenu();
}

void text_viewer_aascii_unicode::pub_hide_search_navigate_buttons(bool status)
{
...
void text_viewer_aascii_unicode::pub_set_global_manager_obj(global_connection_manager *obj)
{
...
{ // public fubction which load any file data in text view section
    ...
    if(global_variable_macOS_appearance_light_mode_bool)
    {
        ...
        ui->textBrowser_text->mergeCurrentCharFormat(fmt);
    }
    else {
        QTextCharFormat fmt;
    ...
    file_data_bytearray = file.readAll();

    file.close();



    if(ui->comboBox_text_type->currentText().contains("ascii", Qt::CaseInsensitive))
        shot_data_in_ascii();
    else if(ui->comboBox_text_type->currentText().contains("unicode", Qt::CaseInsensitive))
        show_data_in_unicode();
    else if(ui->comboBox_text_type->currentText().contains("html", Qt::CaseInsensitive))
        show_data_in_html();
...
{ // highlight keyword hit after searching from text view
    ...
    QTextDocument *document = ui->textBrowser_text->document();
    ...
    if(!list_highlight_cursor.isEmpty())
    {
        ...
        ui->textBrowser_text->setTextCursor(scrollCursor);

        QPalette pallete = ui->textBrowser_text->palette();
        pallete.setColor(QPalette::Highlight, QColor(Qt::yellow));
        pallete.setColor(QPalette::HighlightedText, QColor(Qt::red));
        ui->textBrowser_text->setPalette(pallete);
    }
...
void text_viewer_aascii_unicode::loadFile(const QString &fileName)
{
...
void text_viewer_aascii_unicode::pub_set_recon_file_info(struct_global_recon_file_info obj)
{
    st_globl_recon_file_info_obj = obj;
}

void text_viewer_aascii_unicode::pub_set_source_count_name(QString str)
{
...
{ // change data type from combobox like ascii, unicode, html. we can see the data in text view in the form of these types.
    ...
    if(arg1.contains("ascii", Qt::CaseInsensitive))
        shot_data_in_ascii();
    else if(arg1.contains("unicode", Qt::CaseInsensitive))
        show_data_in_unicode();
    else if(arg1.contains("html", Qt::CaseInsensitive))
        show_data_in_html();
...
{ // show data in ascii format in text view
    ...
    {
        if(file_data_bytearray.at(i) >= 0x20 && file_data_bytearray.at(i) <= 0x7e )
            file_data_ascii.append(file_data_bytearray.at(i));
        else if(file_data_bytearray.at(i) == 0x0a)
            file_data_ascii.append("\n");
        else
            file_data_ascii.append(".");
    }
    ...
    ui->textBrowser_text->clear();
    ui->textBrowser_text->insertPlainText(file_data_ascii);

    ui->textBrowser_text->document()->clearUndoRedoStacks();

    ui->textBrowser_text->moveCursor(QTextCursor::Start);
...
{ // show data in unicode format in text view
    ...
    ui->textBrowser_text->clear();
    // ui->textBrowser_text->insertPlainText(file_data_unicode);
    ui->textBrowser_text->insertPlainText(data_arr);

    ui->textBrowser_text->document()->clearUndoRedoStacks();
    ui->textBrowser_text->moveCursor(QTextCursor::Start);

    //ui->textBrowser_data->setText(file_data_unicode);
}

void text_viewer_aascii_unicode::show_data_in_html()
{

    file_data_unicode = QString::fromLocal8Bit(file_data_bytearray);

    ui->textBrowser_text->setPlainText(file_data_unicode);
    ui->textBrowser_text->document()->clearUndoRedoStacks();
    ui->textBrowser_text->moveCursor(QTextCursor::Start);
...
void text_viewer_aascii_unicode::slot_act_create_tags()
{
    add_tags_module_obj->show();
}

void text_viewer_aascii_unicode::slot_abouttoshow_tags_in_submenu()
{
    create_tags_submenu();
}

void text_viewer_aascii_unicode::slot_right_click_main_menu_triggered(QAction* current_clicked_action)
{
    if(current_clicked_action->text() == QString("Select All"))
    {
        ui->textBrowser_text->selectAll();
    }
}


void text_viewer_aascii_unicode::slot_sub_menu_tags_clicked(QAction* current_clicked_action)
{
...
{ // submenu tags to update tags in database also
    ...
    update_tags_value(tag_name , selected_color);
...
void text_viewer_aascii_unicode::slot_add_tags(QString tag_data , QString colour)
{
...
{ // update tags value in text view and database
    ...
    QString content_file_path = global_narad_muni_class_obj->get_field(MACRO_NARAD_Exported_Content_Location_Text_Viewer_QString).toString();
    ...
    s1.plugin_name = QString(MACRO_Plugin_Name_Text_Viewer);
    s1.tab_name = QString(MACRO_Plugin_Name_Text_Viewer);
    s1.item0 = file_name;
    ...
    global_connection_manager_obj->pub_add_data_in_export_content_string_viewer_and_text_viewer_db(string_viewer_record_list ,tag_data ,MACRO_Plugin_Name_Text_Viewer);
    global_connection_manager_obj->pub_add_tag_for_string_viewer_and_text_viewer(string_viewer_record_list , tag_data);
...
{ // export content and save in database as well
    ...
    if(!info.exists())
    {
        ...
        QString command = "CREATE TABLE 'text_content_info'(INT VARCHAR(1000),bookmark VARCHAR(1000), file_name VARCHAR(1000), file_path VARCHAR(1000),"
                          "file_size VARCHAR(1000), text_data VARCHAR(1000), item2 VARCHAR(1000) , item3 VARCHAR(1000), item4 VARCHAR(1000) , item5 VARCHAR(1000),"
                          "recon_tag_value VARCHAR(1000),"
                          "color_hex VARCHAR(1000), content_file_path VARCHAR(1000),export_file_path VARCHAR(1000) , content_file_name VARCHAR(1000) ,source_count_name VARCHAR(1000), notes VARCHAR(1000), LINKFILENAME VARCHAR(1000),"
