...
def select_all_paths(conn):
    cur = conn.cursor()

    if (os_scheme_internal == "iOS_UID" or os_scheme_internal == "iOS_XML"):
        cur.execute("SELECT id_path , INT , file_size, mime_type  FROM files WHERE fs_module_fire_analysis_run_status='0' And file_size is Not NULL")
    else:
        cur.execute("SELECT filepath , INT , file_size, mime_type  FROM files WHERE fs_module_fire_analysis_run_status = '0' And file_size is Not NULL")
    ...
    for row in rows:
        ...
        if double_slash is True:
            ...
            if output is None:
                continue
            if output == 1:
                out_type = category_fire

            con = sqlite3.connect(weapons_dp_path)
            c = con.cursor()
            c.execute("insert into files(filesystem_record) values (?)", [row[1]])
            con.commit();
...
def set_all_paths():
    ...
    for file_path in comp_pth_list:
        ...
        if (os_scheme_internal == "iOS_UID" or os_scheme_internal == "iOS_XML"):
            if(is_dir == 0):
                cur.execute("SELECT id_path , INT , file_size, mime_type FROM files WHERE fs_module_fire_analysis_run_status='0' And file_size is Not NULL And filepath = '" + file_path + "'")
            else:
                if not(file_path.endswith("/")):
                    file_path = file_path + "/"
                    
                cur.execute("SELECT id_path , INT , file_size, mime_type FROM files WHERE fs_module_fire_analysis_run_status='0' And file_size is Not NULL And filepath LIKE '" + file_path + "%'")
        else:
            if(is_dir == 0):
                cur.execute("SELECT filepath  , INT , file_size, mime_type  FROM files WHERE fs_module_fire_analysis_run_status='0' And file_size is Not NULL And filepath = '" + file_path + "'")

            else:
                if not(file_path.endswith("/")):
                    file_path = file_path + "/"
        
                cur.execute("SELECT filepath  , INT , file_size, mime_type  FROM files WHERE fs_module_fire_analysis_run_status='0' And file_size is Not NULL And filepath LIKE '" + file_path + "%'")
